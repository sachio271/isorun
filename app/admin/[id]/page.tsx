        ...
        return (
          <div key={step.key} className="flex items-center">
            ...
            {i < STATUS_STEPS.length - 1 && (
              <div
                className={`h-0.5 w-8 md:w-14 mx-1 mb-4 rounded ${status > step.key ? "bg-green-400" : "bg-gray-200"}`}
              />
            )}
          </div>
        );
...
const TransactionDetails = () => {
  ...
  const [isDialogFreeOpen, setIsDialogFreeOpen] = useState(false);
  ...
  const handleDeleteParticipant = async () => {
    ...
    try {
      ...
      showToast({
        type: "error",
        title: "Gagal",
        description: "Tidak dapat menghapus peserta.",
      });
    }
  };
  ...
  const handleSetFree = async () => {
    if (!session?.accessToken) return;
    try {
      await setParticipantFree(session.accessToken, participantId);
      fetchTransaction();
      showToast({
        type: "success",
        title: "Berhasil",
        description: "Harga peserta diubah menjadi gratis.",
      });
    } catch {
      showToast({
        type: "error",
        title: "Gagal",
        description: "Tidak dapat mengubah harga peserta.",
      });
    }
  };
  ...
  const handleStatusChange = async (status: string) => {
    ...
    try {
      ...
      showToast({
        type: "error",
        title: "Gagal",
        description: "Tidak dapat mengubah status transaksi.",
      });
    }
    setIsDialogOpen(false);
  };
  ...
  const handleDelete = async () => {
    ...
    try {
      ...
      showToast({
        type: "error",
        title: "Gagal",
        description: "Tidak dapat menolak transaksi.",
      });
    }
  };
  ...
  return (
    <div className="space-y-6">
      ...
      <ConfirmationDialog
        title="Set Gratis"
        description="Apakah anda yakin ingin mengubah harga peserta ini menjadi gratis?"
        handleConfirm={handleSetFree}
        status=""
        isOpen={isDialogFreeOpen}
        onClose={() => setIsDialogFreeOpen(false)}
      />
      ...
      <div className="flex items-center gap-3">
        <Button
          ...
          className="rounded-xl hover:bg-white/60 cursor-pointer"
        >
          ...
          <div className="flex flex-col sm:flex-row gap-2 flex-shrink-0">
            ...
            {transaction?.status === 3 && (
              <>
                <Button
                  variant="outline"
                  className="border-amber-200 text-amber-600 hover:bg-amber-50 cursor-pointer"
                  onClick={() => {
                    setStatus("-2");
                    setIsDialogOpen(true);
                  }}
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-5">
        ...
        <div className="bg-white rounded-2xl shadow-sm border border-gray-100 p-6 flex flex-col gap-3">
          ...
          {(() => {
            const raw = transaction?.transferProof;
            if (!raw || raw === "-") {
              return (
                  ...
                  <TableCell>
                    <div className="flex items-center gap-1">
                      <Button
                        ...
                        size="icon"
                        onClick={() => {
                          setParticipantId(p.id.toString());
                          setIsDialogFreeOpen(true);
                        }}
                        className="text-green-500 hover:text-green-700 hover:bg-green-50 cursor-pointer"
                        title="Set Gratis"
                      >
